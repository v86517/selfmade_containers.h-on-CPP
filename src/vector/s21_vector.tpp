...
template <typename T> T &vector<T>::front() { return *begin(); }

template <typename T> const T &vector<T>::front() const { return *cbegin(); }

template <typename T> T &vector<T>::back() { return *(--end()); }

template <typename T> const T &vector<T>::back() const { return *(--cend()); }

template <typename T> T *vector<T>::data() noexcept { return data_; }
...
template <typename T> size_t vector<T>::max_size() const noexcept {
...
template <typename T> typename vector<T>::iter vector<T>::erase(c_iter pos) {
...
template <typename T> void vector<T>::resize(size_t count, const T &value) {
  if (count > max_size()) {
...
template <typename T> typename vector<T>::iter vector<T>::begin() noexcept {
  return iter(data_);
}

template <typename T> typename vector<T>::iter vector<T>::end() noexcept {
...
template <typename T> void vector<T>::initialize_memory(T *data, size_t sz) {
  size_t k = 0;
  try {
    for (; k < sz; ++k) {
      new (data + k) T();
