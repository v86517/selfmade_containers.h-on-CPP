...
template <typename Key, typename Value> struct map<Key, Value>::BaseNode {
  BaseNode *left;
  BaseNode *right;
  BaseNode *parent;
  int height;
...
template <typename Key, typename Value> map<Key, Value>::~map() noexcept {
  delete_node(static_cast<Node *>(fake_node->left));
  delete fake_node;
}
...
Value &map<Key, Value>::operator[](const Key &key) {
  for (auto it = begin(); it != end(); ++it) {
    if ((*it).first == key) {
      ...
      BaseNode *node =
          static_cast<Node *>(insert_private(it.node, {key, Value()}).first);
...
template <typename Key, typename Value>
typename map<Key, Value>::BaseNode *
map<Key, Value>::rebalance_node(BaseNode *node) {
...
template <typename Key, typename Value>
typename map<Key, Value>::BaseNode *
map<Key, Value>::min_value_node(BaseNode *node) {
...
template <typename Key, typename Value>
typename map<Key, Value>::BaseNode *
map<Key, Value>::rotate_right(BaseNode *node) {
...
template <typename Key, typename Value>
typename map<Key, Value>::BaseNode *
map<Key, Value>::rotate_left(BaseNode *node) {
...
template <typename Key, typename Value>
typename map<Key, Value>::BaseNode *
map<Key, Value>::merge_insert(BaseNode *node, BaseNode *src_node, map &source) {
  if (node == nullptr) {
    ...
    ++size_;
    if (leftmost != fake_node && static_cast<Node *>(src_node)->kv.first <
...
map<Key, Value>::insert(const pair &value) {
...
template <typename Key, typename Value>
std::pair<typename map<Key, Value>::iter, bool>
map<Key, Value>::insert(pair &&value) {
