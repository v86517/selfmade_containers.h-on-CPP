...
template <typename Key> struct multiset<Key>::BaseNode {
  BaseNode *left;
  BaseNode *right;
  BaseNode *parent;
  int height;
...
template <typename Key> struct multiset<Key>::Node : BaseNode {
  key data;
...
template <typename Key> multiset<Key>::~multiset() noexcept {
  delete_node(static_cast<Node *>(fake_node->left));
  delete fake_node;
}
...
template <typename Key> int multiset<Key>::get_height(BaseNode *node) {
  return node ? node->height : 0;
}

template <typename Key> int multiset<Key>::get_balance_factor(BaseNode *node) {
  return node != nullptr ? get_height(node->left) - get_height(node->right) : 0;
}

template <typename Key> void multiset<Key>::delete_node(Node *node) {
  if (node) {
...
template <typename Key>
typename multiset<Key>::BaseNode *
multiset<Key>::rebalance_node(BaseNode *node) {
...
template <typename Key>
typename multiset<Key>::BaseNode *
multiset<Key>::min_value_node(BaseNode *node) {
...
template <typename Key>
typename multiset<Key>::BaseNode *
multiset<Key>::merge_insert(BaseNode *node, BaseNode *src_node,
                            multiset &source) {
  if (node == nullptr) {
...
template <typename Key>
std::pair<typename multiset<Key>::iter, bool>
multiset<Key>::insert(const key &value) {
...
template <typename Key> size_t multiset<Key>::erase(const Key &key) {
  size_t res = 0;
  auto it = begin();
...
template <typename Key> void multiset<Key>::merge(multiset &source) {
  if (this == &source) {
    return;
  }
...
template <typename Key> size_t multiset<Key>::count(const Key &key) const {
  size_t res = 0;
...
template <typename Key>
typename multiset<Key>::c_iter
multiset<Key>::lower_bound(const Key &key) const {
  auto it = cbegin();
...
template <typename Key>
typename multiset<Key>::c_iter
multiset<Key>::upper_bound(const Key &key) const {
  auto it = cbegin();
