...
template <typename Key> struct set<Key>::BaseNode {
  BaseNode *left;
  BaseNode *right;
  BaseNode *parent;
  int height;
...
template <typename Key> struct set<Key>::Node : BaseNode {
  key data;
...
template <typename Key> set<Key>::~set() noexcept {
  delete_node(static_cast<Node *>(fake_node->left));
  delete fake_node;
}
...
template <typename Key> typename set<Key>::iter set<Key>::begin() noexcept {
  return iter(leftmost);
}

template <typename Key> typename set<Key>::iter set<Key>::end() noexcept {
...
template <typename Key> size_t set<Key>::size() const noexcept { return size_; }

template <typename Key> size_t set<Key>::max_size() const noexcept {
  return std::numeric_limits<size_t>::max() / sizeof(Node);
}

template <typename Key> int set<Key>::get_height(BaseNode *node) {
  return node ? node->height : 0;
}

template <typename Key> int set<Key>::get_balance_factor(BaseNode *node) {
  return node != nullptr ? get_height(node->left) - get_height(node->right) : 0;
}

template <typename Key> void set<Key>::delete_node(Node *node) {
  if (node) {
...
template <typename Key>
std::pair<typename set<Key>::BaseNode *, bool>
set<Key>::insert_private(BaseNode *node, const key &value) {
  bool inserted = false;
...
template <typename Key>
std::pair<typename set<Key>::BaseNode *, bool>
set<Key>::insert_private(BaseNode *node, key &&value) {
  bool inserted = false;
...
template <typename Key>
typename set<Key>::BaseNode *
set<Key>::merge_insert(BaseNode *node, BaseNode *src_node, set &source) {
  if (node == nullptr) {
...
template <typename Key> typename set<Key>::iter set<Key>::erase(iter pos) {
  return erase_private(pos, true);
}

template <typename Key> typename set<Key>::iter set<Key>::erase(c_iter pos) {
...
template <typename Key> size_t set<Key>::erase(const Key &key) {
  size_t res = 0;
...
template <typename Key> void set<Key>::merge(set &source) {
  if (this == &source) {
    return;
  }
...
template <typename Key> size_t set<Key>::count(const Key &key) const {
  size_t res = 0;
...
template <typename Key> typename set<Key>::iter set<Key>::find(const Key &key) {
  auto it = begin();
  for (; it != end(); ++it) {
    if (*it == key) {
      return it;
...
std::vector<std::pair<typename set<Key>::iter, bool>>
