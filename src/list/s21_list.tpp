...
list<T>::list(const list &other)
  ...
  try {
    for (ListConstIterator it_other = other.cbegin(); it_other != other.cend();
         ++it_other) {
      push_back(*it_other);
    }
  } catch (...) {
...
template <typename T> const T &list<T>::front() const { return *cbegin(); }

template <typename T> T &list<T>::back() { return *(--end()); }

template <typename T> const T &list<T>::back() const { return *(--cend()); }

template <typename T> typename list<T>::ListIterator list<T>::end() {
  return ListIterator(static_cast<Node *>(&fake_node));
}

template <typename T> typename list<T>::ListIterator list<T>::begin() {
...
template <typename T> size_t list<T>::size() const noexcept { return size_; }

template <typename T> size_t list<T>::max_size() const noexcept {
...
template <typename T> void list<T>::resize(size_t count, const T &value) {
  if (count == 0) {
    clear();
  } else if (count > size_) {
    s21::list<T> new_list;
...
template <typename T> void list<T>::resize(size_t count) { resize(count, T()); }
...
template <typename T> void list<T>::splice(ListConstIterator pos, list &other) {
  if (other.size() > 0) {
    ListIterator it = begin();
...
template <typename T> void list<T>::remove(const T &value) {
